...
import { Bot, CheckCircle2, FileText, Lightbulb, Loader2, MessagesSquare } from "lucide-react";
...
type ModuleKey = "topics" | "plan" | "interview";

const targetTypes: InterviewTargetType[] = [
  "基层干部",
  "村干部/社区干部",
  "驻村干部",
  "普通群众",
    ...
    <main className="min-h-screen px-5 py-6">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-[220px_1fr_280px]">
        ...
        <section className="min-w-0">
          ...
          {activeModule === "topics" ? (
            <TopicModule loading={actionLoading} onGenerate={generateTopics} onSelect={selectTopic} project={project} />
          ) : null}
          {activeModule === "plan" ? (
            <PlanModule
        ...
        <aside>
          <Card className="sticky top-6">
            ...
            <CardContent className="space-y-3 text-sm text-muted-foreground">
              <p>当前支持：生成候选选题、确认主选题、生成调研方案、生成访谈提纲。</p>
              <p>访谈提纲模块暂时只使用 mock 数据，并保存到浏览器本地存储。</p>
              <p>未配置 OPENAI_API_KEY 时，选题和方案也会自动使用 mock 数据。</p>
            </CardContent>
          </Card>
        </aside>
      </div>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-4">
        ...
        <Button disabled={Boolean(loading)} onClick={onGenerate}>
          {loading === "topics" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
          生成5个候选选题
        </Button>
      </CardHeader>
      <CardContent className="grid gap-4">
          ...
          return (
            <div className="rounded-lg border bg-background p-4" key={topic.title}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <Button disabled={Boolean(loading) || selected} onClick={() => onSelect(topic)} variant={selected ? "default" : "outline"}>
                  {selected ? (
                    <>
                      <CheckCircle2 className="mr-2 h-4 w-4" /> 已选为主选题
                    </>
                  ) : (
                    "选择为主选题"
                  )}
              ...
              <div className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                <p>
                  <span className="font-medium">访谈对象：</span>
                  {topic.interviewTargets.join("、")}
                </p>
                <p>
                ...
                <p>
                  ...
                  {topic.feasibilityScore}/10
                </p>
                <p>
                  <span className="font-medium">创新性：</span>
                  {topic.innovationScore}/10
                </p>
                <p>
        ...
        {!project.topics?.length ? (
          <div className="rounded-lg border border-dashed p-8 text-center text-muted-foreground">
            点击按钮生成候选选题，系统会优先调用 AI，没有 key 时返回 mock 数据。
          </div>
        ) : null}
      </CardContent>
    </Card>
...
function PlanModule({
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-4">
        <div>
          <CardTitle>调研方案</CardTitle>
          <p className="mt-2 text-sm text-muted-foreground">基于主选题生成 Markdown 方案，并在右侧预览。</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button disabled={Boolean(loading) || !project.selectedTopic || !planDraft} onClick={onSave} variant="outline">
            {loading === "save-plan" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            保存编辑
          </Button>
          <Button disabled={Boolean(loading) || !project.selectedTopic} onClick={onGenerate}>
            {loading === "plan" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            生成并保存方案
          </Button>
        </div>
      </CardHeader>
      <CardContent className="grid gap-4 xl:grid-cols-2">
        {!project.selectedTopic ? (
          <div className="xl:col-span-2 rounded-lg border border-dashed p-8 text-center text-muted-foreground">
            请先在“选题设计”中选择一个主选题。
          </div>
        ) : (
          <>
            <div>
              <p className="mb-2 text-sm font-medium">Markdown 编辑</p>
              <Textarea className="min-h-[520px] font-mono" onChange={(event) => setPlanDraft(event.target.value)} value={planDraft} />
            </div>
            <div>
              <p className="mb-2 text-sm font-medium">方案预览</p>
              <div className="markdown-preview min-h-[520px] rounded-md border bg-background p-4 text-sm">
                {planDraft ? <ReactMarkdown>{planDraft}</ReactMarkdown> : <p className="text-muted-foreground">生成后将在这里预览。</p>}
              </div>
            </div>
          </>
        )}
...
function InterviewModule({
  ...
  form: InterviewOutlineForm;
  loading: string;
  onFormChange: (form: InterviewOutlineForm) => void;
  onGenerate: () => void;
  onSave: () => void;
  outlineDraft: string;
  savedAt: string;
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          <CardTitle>访谈提纲</CardTitle>
          <p className="mt-2 text-sm text-muted-foreground">选择访谈对象并填写约束，生成可编辑的结构化访谈提纲。</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button disabled={Boolean(loading) || !outlineDraft} onClick={onSave} variant="outline">
            保存提纲
          </Button>
          <Button disabled={Boolean(loading)} onClick={onGenerate}>
            {loading === "interview" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            生成访谈提纲
          </Button>
        </div>
      </CardHeader>
        ...
        <div className="grid gap-4 rounded-lg border bg-background p-4 md:grid-cols-2">
          <div className="grid gap-2">
            ...
            <select
              ...
              onChange={(event) => updateForm("targetType", event.target.value as InterviewTargetType)}
          ...
          {form.targetType === "自定义对象" ? (
            <div className="grid gap-2 md:col-span-2">
              <Label>自定义对象</Label>
              <input
                className="h-10 rounded-md border border-border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-primary/30"
                onChange={(event) => updateForm("customTarget", event.target.value)}
                placeholder="如：返乡创业青年、非遗传承人、合作社负责人"
          ...
          <label className="flex items-center gap-2 text-sm font-medium">
            <input
              checked={form.hasSensitiveTopics}
              className="h-4 w-4"
              onChange={(event) => updateForm("hasSensitiveTopics", event.target.checked)}
              type="checkbox"
            />
            涉及敏感议题
          </label>
          <div className="grid gap-2 md:col-span-2">
            <Label>希望重点了解的问题</Label>
            <Textarea
              onChange={(event) => updateForm("focusQuestions", event.target.value)}
              placeholder="例如：政策落地难点、群众真实感受、项目可持续性、资源协同机制等"
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <div>
            <div className="mb-2 flex items-center justify-between">
              <p className="text-sm font-medium">提纲编辑</p>
          ...
          <div>
            <p className="mb-2 text-sm font-medium">提纲预览</p>
            <div className="markdown-preview min-h-[520px] rounded-md border bg-background p-4 text-sm">
              {outlineDraft ? <ReactMarkdown>{outlineDraft}</ReactMarkdown> : <p className="text-muted-foreground">生成后将在这里预览。</p>}
            </div>
          </div>
        </div>
      </CardContent>
...
function mockInterviewOutline(project: ProjectRecord, form: InterviewOutlineForm): InterviewOutline {
  ...
  return {
    ...
    basicQuestions: [
      `请您简单介绍一下自己的工作/生活背景，以及与“${project.theme}”相关的经历。`,
      `您平时接触${project.theme}相关事务的频率高吗？主要通过哪些渠道了解？`,
      "在您看来，近几年当地相关工作或生活环境发生了哪些变化？"
    ],
    coreQuestions: [
      `围绕“${focus}”，您认为最值得关注的问题是什么？`,
      `从${target}的视角看，目前最明显的成效和不足分别是什么？`,
      "哪些主体在其中发挥了关键作用？他们之间的协同是否顺畅？",
      "如果要让相关工作更可持续，您认为最需要优先改进的是什么？"
    ],
    followUps: [
      "您刚才提到的这个现象，有没有具体案例可以展开说说？",
      "这个问题大概从什么时候开始变得明显？背后可能有哪些原因？",
      "不同群体对这件事的看法是否一致？分歧主要在哪里？",
      "如果资源有限，您觉得最先应该做哪一件事？为什么？"
    ],
    sensitiveAlternatives: form.hasSensitiveTopics
      ? [
          "将“是否存在矛盾/冲突”替换为“推进过程中有哪些需要协调的地方”。",
          "将“责任在谁”替换为“哪些环节还有进一步优化空间”。",
          "将“群众是否不满意”替换为“大家反馈比较集中的期待是什么”。"
        ]
